...
describe("RoundCard", () => {
  it("scopes the mode control and time presets to the Work section", () => {
    ...
    expect(
      within(work).getByRole("group", { name: "Round 1 work mode" })
    ).toBeInTheDocument()
    expect(
      within(work).getByRole("button", { name: "45s", pressed: true })
    ).toBeInTheDocument()
    expect(
      within(rest).getByRole("button", { name: "20s", pressed: true })
    ).toBeInTheDocument()
    expect(
      screen.queryByRole("button", { name: "Increase Pace" })
  ...
  it("shows Pace only in Reps mode and keeps Rest available", async () => {
    ...
    expect(
      screen.getByRole("button", { name: "Increase Pace" })
    ).toBeInTheDocument()
    const rest = screen.getByRole("region", { name: "Rest" })
  ...
  it("renders the approved preset values", () => {
    ...
    expect(
      within(work).getByRole("button", { name: "30s" })
    ).toBeInTheDocument()
    expect(
      within(work).getByRole("button", { name: "45s" })
    ).toBeInTheDocument()
    expect(
      within(work).getByRole("button", { name: "60s" })
    ).toBeInTheDocument()
    expect(
      within(rest).getByRole("button", { name: "10s" })
    ).toBeInTheDocument()
    expect(
      within(rest).getByRole("button", { name: "20s" })
    ).toBeInTheDocument()
  })
})
